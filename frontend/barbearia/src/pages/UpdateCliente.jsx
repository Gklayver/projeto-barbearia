...
export default function UpdateCliente() {
  ...
  const onSubmit = async (e) => {
    ...
    const data = {
      telefone: removeMask(phoneValue),
      email,
      genero: gender,
      nome: name,
    };
    toast.promise(api.put(`/clientes/${params.id}`, data), {
      loading: "Carregando...",
  ...
  async function fetchUserById() {
    ...
    setPhone(formatPhoneNumber(data.telefone));
    setEmail(data.email);
    setName(data.nome);
  ...
  return (
    <Form title="Atualizar cliente" onSubmit={onSubmit}>
      <VStack spacing="8">
        <SimpleGrid
